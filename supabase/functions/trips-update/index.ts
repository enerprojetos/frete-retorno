...
function toLineStringEWKT(coords: Array<[number, number]>) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const actorUserId = body.actorUserId as string
    const profile = (body.profile ?? 'driving-hgv') as string
    const corridorRadiusM = Number(body.corridorRadiusM ?? 50_000)
    ...
    if (!coords || coords.length < 2) {
      return new Response(JSON.stringify({ error: 'INVALID_ROUTE_GEOMETRY' }), {
        status: 400,
        headers: { ...corsHeaders(), 'content-type': 'application/json' },
      })
    }
    ...
    const { data, error } = await supabaseAdmin.rpc('update_trip', {
      p_trip_id: tripId,
    ...
    if (error) {
      return new Response(JSON.stringify({ error: 'DB_ERROR', detail: error }), {
        status: 500,
        headers: { ...corsHeaders(), 'content-type': 'application/json' },
      })
    }
